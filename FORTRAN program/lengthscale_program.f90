...
! testcase.vtk - vtk file with all voxels in the microstructure 
!				 NOTE: this vtk file is not required for the grainsize subroutine
!				       when using a crystal plasticity based software.
!                      It is included here to allow this to be a
!                      standalone program.
...
!where you can find the tool Lengmorph to create the binary files.
!To create the vtk file, you can use the vtk export filter from 
!Dream3D and remove the headers to the file keeping only the grain ids.
...
!read in voxels
    ...
    call totalvox(voxarray)

!read in data
    call EL_CENTROID(elcent,totalels)

    call UEXTERNALDB(NODEX,NODEY,NODEZ,nodeout,totalfeat)
...
!developing slip plane normals and slip directions
    ...
    call grainsize(coords,nodeout,totalfeat,nodex,nodey,nodez,        &
...
subroutine grainsize(coords,nodeout,totalfeat,nodex,nodey,nodez,      &
...
real*8, allocatable :: elcent(:,:), boundgrain(:,:),             &
    ...
    uniqgraindist(:),oriensh(:,:),zrot(:,:),slpdirrotate(:,:),   &
...
real*8 :: coords(3),vectotal(3),                                  &
    ...
    twovoxdist(3),centrodes(3),totalrota(3,3),sortmindistsing,   &
    uniqgraindistsing
real :: start, finish,starttotal,finishtotal
 dimension :: SLPDIR1(3,ND), SLPNOR1(3,ND)
! Dimension :: ispdir(3),ispnor(3),                               &
!    SLPDIR2(3,50), SLPNOR2(3,50),nslip
...
!finding the closest bounding grain and using this only
...
!using the index of unique grains, the corresponding distance value can be extracted
    ...
    uniqgraindist=sortmindist(uniindex)
...
! give the closest grain the label grainb       
...
subroutine eulercosmatrix(orient,totalrot)
    ...
    SUBROUTINE UEXTERNALDB(NODEX,NODEY,NODEZ,nodeout,totalfeat)
...
! 
  ...
  SUBROUTINE totalvox2(voxarray)
!reads in vox array data
! 
     integer*8 nr,nc
     real*8 :: numrowval,numcolval
     REAL*8, allocatable :: voxarray(:,:),rowdata(:)

     open(5300, FILE="vox.bin",form='unformatted', 		    &
                status='old',access='stream')
  ...
  END subroutine totalvox2


	
	!just added this function since norm2 is available in this visual studio
...
!----------------------------------------------------------------------
 ...
 !!!!!!!!!!!!!!!!!!!!!REFERENCE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!     
 ! The following subroutine was extracted from the following reference:
...
!
     ...
     SUBROUTINE totalvox(voxarray)
! read in the centroid coordinates for each voxel
      
     integer*8 i,col,grains,n_rows,n_cols
     integer totalvals
     REAL*8, allocatable :: voxarray(:,:)
     CHARACTER(1000000) :: dummyrow


!    initalise row values
     iostat=0
     n_rows=0

     open(234, FILE="testcase.vtk", 		   &
                status='old', action='read')
! count rows     
     row_count_loop: DO
	READ(234, '(a)', IOSTAT=IO_Status) dummyrow
	IF(IO_Status < 0) THEN
		EXIT row_count_loop
	END IF
	n_rows = n_rows + 1
     ...
     ALLOCATE(voxarray(n_rows,20))

     DO i=1,n_rows
	READ(234,*) (voxarray(i, col), col=1,20)
     END DO
	totalvals=n_rows

  END subroutine totalvox
